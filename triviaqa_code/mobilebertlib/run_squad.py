...
from mobilebertlib import tokenization
...
flags.DEFINE_string(
    "data_dir", None,
    "The data directory where the tf_record checkpoints will be written.")
...
class LiteRunner(object):
  ...
  def run(self, input_ids, input_mask, segment_ids):
  ...
  def predict(self, input_fn, params):
    """Predicts and returns a list of results from input_fn."""
    all_results = []

    # TODO(tianlin): To support TF 2.0 style. Now TF 1.x.
    ...
    for feature in generate_features():
      ...
      start_logits, end_logits = self.run(input_ids, input_mask, segment_ids)
      unique_id = int(feature["unique_ids"][0])
...
def main(_):
  ...
  if FLAGS.do_predict:
    ...
    eval_writer = FeatureWriter(filename=eval_file, is_training=False)
    eval_features = []
    ...
    if FLAGS.lite_model_path:
      ...
      params = {"batch_size": 1}
      all_results = lite_runner.predict(predict_input_fn, params)
    else:
      # Use TF estimator for prediction.
      ...
      if FLAGS.activation_quantization:
        ...
        def representative_dataset_gen():
          ...
          for example in examples:
            yield [np.array([example.input_ids], dtype=np.int32),
                   np.array([example.input_mask], dtype=np.int32),
                   np.array([example.segment_ids], dtype=np.int32)]

        converter.representative_dataset = representative_dataset_gen

    tflite_model = converter.convert()
    tflite_file = os.path.join(FLAGS.export_dir, "model_quant.tflite")
    with tf.gfile.GFile(tflite_file, "wb") as f:
